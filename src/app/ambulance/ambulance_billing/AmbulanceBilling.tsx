...
import BILLS from "../../../assets/ambulance_billings.json";
...
const AmbulanceBilling: React.FC = () => {
  const heading = (
    <span>
      <span className="text-[#999999]">Ambulance</span> &gt;{" "}
      <span className="text-[#4D4D4D]">Ambulance Billing Search</span> &gt;{" "}
      <span className="text-[#4D4D4D]">Ambulance Billing</span>
    </span>
  );
  ...
  const handleSearch = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const filteredBills = useMemo(() => {
      console.log('Sample data date:', localBills[0]?.date);
      ...
      return localBills.filter((bill) => {
        ...
        normalizeDate(bill.date) === normalizeDate(selectedDate);
  ...
  return (
    <div className="px-6 py-3 shadow-lg rounded-xl">
      ...
      >
        <span className="font-semibold text-xs">Rate Chart</span>
        {isEditModeOn === false ? (
          <button
            onClick={(e) => {
              e.stopPropagation(); 
