...
import BILLS from "@/assets/diagnostic_reports.json";
...
const DiagnosticReport: React.FC = () => {
  const heading = (
    <span>
      ...
      <span className="text-[#475467]">Diagnostics Report Search</span> &gt;{" "}
      <span className="text-[#475467]">Diagnostics Report</span>
    </span>
  );
  ...
  const clearSelectedFilter = (type: string) => {
    if (type === "status") {
      ...
      setStatusFilters({ COMPLETED: false, BOOKED: false, INPROGRESS: false });
    } else if (type === "date") {
  ...
  const filteredBills = useMemo(() => {
    return localBills.filter((bill) => {
      ...
      const searchMatch =
        !searchTerm || bill.slNo.toLowerCase().includes(searchTerm.toLowerCase());

      const dateMatch =
        !selectedDate || 
        bill.testDate === selectedDate || 
        bill.deliveryDate === selectedDate;
          ...
          <div className="relative flex items-center gap-2">
            ...
            {selectedStatus && (
              <div className="flex items-center gap-2 bg-white px-3 py-1 rounded-lg border border-[#999999]">
                ...
                <button
                  onClick={() => clearSelectedFilter("status")}
            ...
            <div className="flex items-center gap-2">
              ...
              {selectedDate && (
                <div className="flex items-center gap-2 bg-white px-3 py-1 rounded-lg border border-[#999999]">
                  ...
                  <button
                    onClick={() => clearSelectedFilter("date")}
                    className="text-black hover:text-pink-500"
                  >
                    ×
                  </button>
