...
import BILLS from "@/assets/oxygen_bills.json";
...
const OxygenBilling: React.FC = () => {
  const heading = (
    <span>
      <span className="text-[#999999]">Oxygen</span> &gt;{" "}
      <span className="text-[#475467]">Oxygen Billing Search</span> &gt;{" "}
      <span className="text-[#475467]">Oxygen Billing</span>
    </span>
  );
  ...
  const filteredBills = useMemo(() => {
    console.log('Sample start date:', localBills[0]?.startDate);
    console.log('Sample end date:', localBills[0]?.endDate);
    ...
    return localBills.filter((bill) => {
          ...
          normalizeDate(bill.startDate) === normalizeDate(selectedDate) || 
          normalizeDate(bill.endDate) === normalizeDate(selectedDate);
