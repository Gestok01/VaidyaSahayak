...
import BILLS from "@/assets/doctor_appointment_list.json";
...
const AppointmentBilling: React.FC = () => {
  const searchParams = useSearchParams();
  const host = process.env.NEXT_PUBLIC_HOST;
  const getQueries = () => {
    const patient_name = searchParams.get("patientName");

  }
  const heading = (
    <span>
      <span className="text-[#999999]">Doctor</span> &gt;{" "}
      <span className="text-[#4D4D4D]">Search Appointment</span> &gt;{" "}
      <span className="text-[#4D4D4D]">Appointment Billing</span>
    </span>
  );
  ...
  const [paymentStatusFilters, setPaymentStatusFilters] = useState<{ [key: string]: boolean }>({
    PAID: false,
    UNPAID: false,
    PARTIAL: false,
  });
  ...
  const clearSelectedStatus = (type: string) => {
    if (type === "status") {
      ...
      setStatusFilters({ COMPLETED: false, BOOKED: false, CANCELLED: false });
...
const handleDateChange = (e: React.ChangeEvent<HTMLInputElement>) => {
  ...
  const filteredBills = useMemo(() => {
    return localBills.filter((bill) => {
      ...
      const searchMatch =
        !searchTerm || bill.patientName.toLowerCase().includes(searchTerm.toLowerCase());

      const dateMatch =
        !selectedDate || bill.appointmentDate === selectedDate;

      return StatusMatch && paymentStatusMatch && searchMatch && dateMatch;
    });
  }, [localBills, StatusFilters, paymentStatusFilters, searchTerm, selectedDate]);
  ...
  const fetchTableData = async () => {
        const response = await axios.get(`${host}/api/v1/appointment/get_all_appointments`);
        const appointmentList = response.data.data.appointments;
    
       const cData =  appointmentList.map((app,idx) => (
          {
        ...
        appointmentTime: app.appointment_time,
          totalBill: app.total_bill,
          paymentStatus: app.payment_status , 
          status: app.status
          }
        ))
        console.log(cData);
        setAppointments(cData);
  }
  useEffect(()=>{
    fetchTableData();
  },[])
      ...
      <div className="flex justify-between items-center mb-6 flex-wrap gap-4">
        <div className="relative flex items-center gap-2">
          ...
          <div className="relative flex items-center gap-2">
            ...
            >
              <Image
                src={activeDropdown === "paymentStatus" ? SORT_ICON_WHITE : SORT_ICON_PINK}
                width={20}
                height={20}
                alt="Sort Icon"
                className="mr-2"
            ...
            {selectedPaymentStatus && (
              <div className="flex items-center gap-2 bg-white px-3 py-1 rounded-lg border border-[#999999]">
                <span className="text-[14px] text-[#999999]">{selectedPaymentStatus}</span>
                <button
