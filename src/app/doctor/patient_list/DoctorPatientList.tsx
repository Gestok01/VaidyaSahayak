...
import BILLS from "@/assets/patientList.json";
...
import BillModal from "../../doctor/patient_list/BillingModal";  
...
const DoctorPatientList: React.FC = () => {
  const heading = (
    <span>
      ...
      <span className="text-[#4D4D4D]">Patient List Search</span> &gt;{" "}
      <span className="text-[#4D4D4D]">Patient List</span>
    </span>
  );
  ...
  const filteredBills = useMemo(() => {
    return localBills.filter((bill) => {
      ...
      const dateMatch = !selectedDate || 
        normalizeDate(bill.appointmentDate) === selectedDate;
