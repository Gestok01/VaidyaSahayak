...
import OXYGEN_COSTING_DATA from "../../../../../assets/oxygen_costing_data.json";

const OxygenCostingTable: React.FC = () => {
  const heading = (
    <span>
      <span className="text-pink-400">₹</span>{" "}  
      <span className="text-[#999999]">Costing</span> &gt;{" "}
      <span className="text-[#4D4D4D]">Get Oxygen Costing Search</span> &gt;{" "}
      <span className="text-[#4D4D4D]">Get Oxygen Costing</span>
    </span>
  );
  ...
  const [oxygenData, setOxygenData] = useState(OXYGEN_COSTING_DATA);
  const [currentPage, setCurrentPage] = useState(1);
  const itemsPerPage = 10;

  // Filter data based on search term
  const filteredData = useMemo(() => {
    if (!searchItemName) return oxygenData;
    return oxygenData.filter(item =>
      item.itemName.toLowerCase().includes(searchItemName.toLowerCase())
    );
  }, [oxygenData, searchItemName]);
  ...
  // Pagination logic - use filteredData instead of oxygenData
  ...
  return (
    <div className="px-6 py-3 shadow-lg rounded-xl">
      ...
      <div className="flex justify-between items-center bg-gray-50 px-6 py-2 border-x-2 border-t-2 rounded-t-md border-gray-200">
        <span className="font-semibold text-xs font-normal">Oxygen Costing Details</span>
        {!isEditModeOn ? (
          <button
...
export default OxygenCostingTable;